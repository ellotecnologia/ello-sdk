...
uses
  Classes, SysUtils, Windows, Forms, ShellAPI, Dialogs, StdCtrls, Buttons, ExtCtrls, Graphics, Controls,
...
type
  ...
  public
    constructor Create(const AMessage: string; AType: TMsgDlgType; AButtons: TMsgDlgButtons; AHelpContext: Longint; ADefResult: Word; ASeconds: Integer = 10);
  ...
  private
    ...
    function ConvertBuildEventStringToIniFormat(
      AStrings: TStringList): String;
    procedure ConvertIniFormatStringToBuildEvent(AText: String;
  ...
  public
    ...
    procedure LoadProjectEvents(AFileName: String);
    procedure SaveProjectEvents(AFileCheck: Boolean = False);
    function CopyProjectEvents(ASourceFile: String): Boolean;
    ...
    property PreBuildEvents : TStringList read FPreBuildEvents write SetPreBuildEvents;
    property PostBuildEvents : TStringList read FPostBuildEvents write SetPostBuildEvents;
    property PostBuildOption: TPostBuildOption read FPostBuildOption write FPostBuildOption;

    property FileName: String read GetFileName Write SetFileName;
    property PreBuildEnabled: Boolean read FPreBuildEnabled Write FPreBuildEnabled;
    property PostBuildEnabled: Boolean read FPostBuildEnabled Write FPostBuildEnabled;
  end;
...
function RunProgram(psCmdLine : String; AWaitUntil : TWaitOption) : TProcessInformation;
...
function StringContains(ASource: String; AStringArray: array of string; AIgnoreCase: Boolean = True): Boolean;
function ContainsString(ASource: String; AStringArray: array of string; AIgnoreCase: Boolean = True): Boolean;
function IfThen(ACondition: Boolean; ATrueValue, AFalseValue: Variant): Variant;
function ValidatePath(APath: String): String;
{$IFNDEF D6_UP}
...
function GetConfirmation(const AFormat: string; AParams: array of const): Boolean; overload;
...
function GetProjectOptionsNames(AOptions: IOTAOptions; AList: TStrings; AIncludeType: Boolean = False): Boolean;
function GetProjectOptionValue(AOptionName: String): String;
...
uses
  Registry, {$IFDEF D6_UP} Variants, {$ENDIF} IniFiles, uBuildOptionsForm;
...
begin
  ...
  try
    for I := Low(Names) to High(Names) do
      {if AIncludeType then  AList.Add(Names[i].Name + ': ' + GetEnumName(TypeInfo(TTypeKind), Ord(Names[i].Kind)))
      else}
        AList.Add(Names[i].Name + '=' + VarToStr(AOptions.Values[Names[I].Name]));
  finally
    Names := nil;
...
begin
  Result := '';
  ProjectOptions := GetActiveProjectOptions(nil);
  if Assigned(ProjectOptions) then
  begin
    Result := VarToStr(ProjectOptions.Values[AOptionName]);
  end;
end;
...
begin
  ...
  Result := (EditorServices.TopView.GetEditWindow.Form.FindComponent('EditorLocalMenu') as TPopupMenu);
end;
...
function GetBuildMacroList: TStringList;
var
  projExt, projName, sUserProfile,
  projFileName, sTemp: String;
begin
  Result := GetCustomMacros;
  projFileName := GetCurrentProjectFileName;
  projName := ExtractFileName(projFileName); projName := ChangeFileExt(projName, '');
  projExt := ExtractFileExt(projFileName);
  sUserProfile := GetEnvVar('UserProfile');

  Result.Add(Format('ProjectName=%s', [projName]));
  Result.Add(Format('ProjectFileName=%s', [ExtractFileName(projFileName)]));
  Result.Add(Format('ProjectPath=%s', [ExtractFilePath(projFileName)]));
  Result.Add(Format('ProjectExt=%s', [ExtractFileExt(projFileName)]));
  sTemp := ValidatePath(ExpandFileName(ExtractFilePath(projFileName) + GetProjectOptionValue('UnitOutputDir')));
  Result.Add(Format('UnitOutputDir=%s', [sTemp]));
  Result.Add(Format('TargetName=%s', [projName]));
  Result.Add(Format('TargetFileName=%s.exe', [projName]));
  Result.Add(Format('TargetLibraryFileName=%s.dll', [projName]));
  sTemp := ValidatePath(ExpandFileName(ExtractFilePath(projFileName) + GetProjectOptionValue('OutputDir')));
  Result.Add(Format('TargetPath=%s', [sTemp]));
  Result.Add('ExeExt=.exe');
  Result.Add('LibExt=.dll');
  Result.Add('WinDir=C:\Windows\');
  Result.Add('SysDir=C:\Windows\System32\');
  Result.Add('TMan=' + sUserProfile +'\Applic~1\Morgan~1\TempestApps\TempestPlus\');
  Result.Add('TPlus=C:\MSDE\Dev\Commodities\tempest\TempestPlus2\');
  Result.Add('TPlusClient=C:\MSDE\Dev\Commodities\tempest\TempestPlus2\dev\Client\Bin\');
  Result.Add('TPlusServer=C:\MSDE\Dev\Commodities\tempest\TempestPlus2\dev\Server\Bin\');
  { Environment Variables }
  sTemp := ValidatePath(GetEnvVar('TEMP'));
  if (sTemp = EmptyStr) then sTemp := ValidatePath(GetEnvVar('TMP'));
  Result.Add(Format('TempDir=%s', [sTemp]));
...
begin
  ...
  begin
    Result := IfThen(AIgnoreCase, ( 0 = CompareText(ASource, AStringArray[Loop])), (ASource = AStringArray[Loop]));
...
begin
  ...
  begin
    ...
    for Loop := Low(AStringArray) to High(AStringArray) do  AStringArray[Loop] := LowerCase(AStringArray[Loop]);
  end;
...
begin
  ...
  if CreateProcess(nil, PChar(sCmdLine), nil, nil, true, 0, nil, nil, StartInfo, Result) then
  begin
...
function MessageDlgEx(const AMessage: string; AType: TMsgDlgType; AButtons: TMsgDlgButtons; AHelpContext: Longint; ADefResult: Word; ASeconds: Integer = 10): Word;
begin
  with TAutoCloseMessage.Create(AMessage, AType, AButtons, AHelpContext, ADefResult, ASeconds) do
  try
    Result := Execute;
  finally
    Free;
...
begin
  MessageBox(0, PAnsiChar(AMessage), PAnsiChar(ACaption), MB_OK or MB_ICONEXCLAMATION);
end;
...
begin
  Result := (MessageDlgEx(AMessage, mtConfirmation, [mbYes, mbNo], 0, IDNO) = IDYES);
end;

function GetConfirmation(const AFormat: string; AParams: array of const): Boolean;
begin
  Result := GetConfirmation(Format(AFormat, AParams));
end;

{ TAutoCloseMessage }
constructor TAutoCloseMessage.Create(const AMessage: string; AType: TMsgDlgType; AButtons: TMsgDlgButtons; AHelpContext: Longint; ADefResult: Word; ASeconds: Integer = 10);
begin
  inherited Create;
...
const
  CReturnStr: array[0..10] of string = ('mrNone', 'mrOk', 'mrCancel', 'mrAbort', 'mrRetry', 'mrIgnore', 'mrYes', 'mrNo', 'mrAll', 'mrNoToAll', 'mrYesToAll');
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        //with FForm.Components[I] as TButton do ShowMessageFmt('%s - %d - Default: %s', [Caption, ModalResult, ResultStr(FResult)]);
...
procedure TAutoCloseMessage.DoOnDialogClose(Sender: TObject; var Action: TCloseAction);
begin
  if Assigned(FForm) then
...
begin
  ...
  try
    ...
    FAutoSaveInterval := ReadInteger(REG_BUILD_OPTIONS, 'Interval', FAutoSaveInterval);
  finally
    Free;
  end;
end;
...
function TBuildOptions.ConvertBuildEventStringToIniFormat(AStrings: TStringList): String;
var
  I: Integer;
begin
  Result := '';
  ...
  begin
    if (Result = EmptyStr) then
      Result := Trim(AStrings[I])
    else
      Result := Result +'|' + Trim(AStrings[I]);
  end;
end;

procedure TBuildOptions.ConvertIniFormatStringToBuildEvent(AText: String; var AStrings: TStringList);
begin
  AStrings.Clear;
  AStrings.Text := StringReplace(AText, '|', #13#10, [rfReplaceAll]);
end;

function TBuildOptions.CopyProjectEvents(ASourceFile: String): Boolean;
begin
  ...
  begin
    ...
    begin
      try
        ConvertIniFormatStringToBuildEvent(Trim(ReadString('Build Events', 'PreBuild', '')), FPreBuildEvents);
        ConvertIniFormatStringToBuildEvent(Trim(ReadString('Build Events', 'PostBuild', '')), FPostBuildEvents);
        PostBuildOption := TPostBuildOption(ReadInteger('Build Events', 'BuildEvent', 0));
...
procedure TBuildOptions.LoadProjectEvents(AFileName: String);
begin
  LogText('TBuildOptions.LoadProjectEvents(FileName: %s, NewFileName: %s)', [FileName, AFileName]);
  ClearEvents;
  FileName := AFileName;
  CopyProjectEvents(FileName);
  LogText('TBuildOptions.LoadProjectEvents(AFileName: %s)', [FileName]);
end;

procedure TBuildOptions.SaveProjectEvents(AFileCheck: Boolean = False);
begin
  LogText('TBuildOptionExpert.SaveProjectEvents (AFileCheck: %s)', [BoolToStr(AFileCheck, True)]);
  if (EmptyStr <> FileName) then
  begin
    if (AFileCheck) then
    begin
    ...
    begin
      try
        WriteString('Build Events', 'PreBuild', ConvertBuildEventStringToIniFormat(PreBuildEvents));
        WriteString('Build Events', 'PostBuild', ConvertBuildEventStringToIniFormat(PostBuildEvents));
...
var
  sExt: String;
begin
  LogText('In - TBuildOptions.SetFileName(FileName: %s, NewValue: %s)', [FFileName, Value]);
  ...
  LogText('Out - TBuildOptions.SetFileName(FileName: %s, NewValue: %s)', [FFileName, Value]);
end;
...
begin
  Result := EmptyStr;
  if (FFileName = '') then FFileName := GetCurrentProjectFileName;
  if (FFileName <> '') then
  begin
