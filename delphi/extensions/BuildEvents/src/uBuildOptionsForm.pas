...
uses
  Windows, Messages, SysUtils, {$IFDEF D6_UP}Variants, {$ENDIF} Classes, Graphics, Controls, Forms, Dialogs, StdCtrls,
  Buttons, ComCtrls, ExtCtrls, uBuildMisc, Spin;
...
type
  TBuildOptionsForm = class(TForm)
    ...
    procedure cbFontNamesDrawItem(AControl: TWinControl; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
    ...
    procedure pgcOptionsChanging(Sender: TObject; var AllowChange: Boolean);
  private
    ...
    procedure InitProductAuthorColors;
  public
    { Public declarations }
...
uses uBuildCommandEditor, uBuildOptionExpert, uBuildEngine, uColorConsts, ShellAPI;
...
procedure TBuildOptionsForm.InitProductAuthorColors;
var
 nRandColor: Integer;
begin
  nRandColor := Random(High(CustomColorsArray));
  LabelProduct.Font.Color := CustomColorsArray[nRandColor];

  nRandColor := Random(High(CustomColorsArray));
  LabelAuthor.Font.Color := CustomColorsArray[nRandColor];
end;
...
begin
  mPreBuildEvent.Lines.Text := ShowBuildCommandEditor('Pre', mPreBuildEvent.Lines.Text );
end;

procedure TBuildOptionsForm.mPostBuildEventDblClick(Sender: TObject);
begin
  mPostBuildEvent.Lines.Text := ShowBuildCommandEditor('Post', mPostBuildEvent.Lines.Text );
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      cbPostBuildEvents.ItemIndex := Integer(BuildOptionExpert.Options.PostBuildOption);
...
procedure TBuildOptionsForm.cbFontNamesDrawItem(AControl: TWinControl; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
begin
...
begin
  ShellExecute(0, 'open', 'http://fwdirectory.ms.com/itsmg/dman/fcgi-bin/d-lite.cgi?skin=default&dn=msfwid%3D680616%2Cou%3Dpeople%2Co%3Dmorgan%20stanley', nil, nil, SW_NORMAL);
end;

procedure TBuildOptionsForm.pgcOptionsChanging(Sender: TObject; var AllowChange: Boolean);
begin
  InitProductAuthorColors();
end;
